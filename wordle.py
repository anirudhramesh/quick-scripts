...
known_positions = {1: 'e'} # dict with format index:letter with index in range(0, 5)
unknown_positions = 'rek'
should_not_match = {2: 'e'} # positions that you know don't contain that specific letter
...
def use_pandas():
    ...
    if should_not_match:
        df = df[reduce(and_, [~(df[pos]==should_not_match[pos]) for pos in should_not_match])]
...
def use_regex():
    ...
    should_not_match_regex = ''.join([should_not_match[pos] if pos in should_not_match else '.' for pos in range(5)])
    
    filtered = []
    for w in _5_letter_words:
        if re.search(exclude_letters_regex, w) and re.search(unknown_positions_regex, w) and re.search(known_positions_regex, w) and not re.search(should_not_match_regex, w):
            filtered.append(w)
    return filtered
...
def just_do_it():
    ...
    for w in _5_letter_words:
        if not any(l in exclude_letters for l in w) and all(l in w for l in unknown_positions) and all(w[pos]==known_positions[pos] for pos in known_positions) and \
            not any(w[pos]==should_not_match[pos] for pos in should_not_match):
            filtered.append(w)
